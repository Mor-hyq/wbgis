    ...
    >
      <template slot="search">
        <el-form ref="searchForm" :model="searchForm" :size="$btnSize" inline>
          ...
          <el-form-item prop="name" :label="mylang.realName">
            <el-input v-model="searchForm.name" :placeholder="`请输入${mylang.realName}`" clearable />
          </el-form-item>
          <el-form-item prop="order" :label="mylang.number">
            <el-input v-model="searchForm.order" :placeholder="`请输入${mylang.number}`" clearable />
          </el-form-item>
          <el-form-item prop="commander" :label="mylang.commander">
            <el-input v-model="searchForm.commander" :placeholder="`请输入${mylang.commander}`" clearable />
          </el-form-item>
      ...
      >
        ...
        <el-table-column
          ...
          prop="depart_name"
          show-overflow-tooltip
          :label="mylang.signdepartment"
        />
        <el-table-column
          align="center"
          prop="register"
          show-overflow-tooltip
          :label="mylang.register"
        />
        <el-table-column
          align="center"
          prop="create_time"
...
<script>
import { getStaffList, deleteStaff } from '@/api/emergency'
...
export default {
  ...
  methods: {
    initTableData({
      ...
      this.getList({ page, paginate, name, order, unit, commander })
    },
    async getList({
      order = '',
      name = '',
      ...
      try {
        const { code, data } = await getStaffList({
          page, paginate, name, order, unit, commander
        })
    ...
    handleDeleteItem(row) {
      this.$confirm('即将删除您选中的' + this.mylang.emergencyStaff + '，确认吗？', '删除' + this.mylang.emergencyStaff, {
    ...
    async deleteData({ delete_ids } = {}) {
      ...
      try {
        const { code } = await deleteStaff({ delete_ids })
        loading.close()
        if (code === 200) {
          this.$message({
            type: 'success',
          ...
          this.getList({
            ...
            commander: this.realSearch.commander
          })
        }
      } catch (error) {
        loading.close()
