...
<script>
import { getCarList, deleteCar } from '@/api/emergency'
...
export default {
  ...
  methods: {
    initTableData({
      ...
      this.getList({ page, paginate, order, unit })
    },
    async getList({
      order = '',
      unit = '',
    ...
    handleDeleteItem(row) {
      this.$confirm('即将删除您选中的' + this.mylang.emergencyCars + '，确认吗？', '删除' + this.mylang.emergencyCars, {
    ...
    async deleteData({ delete_ids } = {}) {
      ...
      try {
        const { code } = await deleteCar({ delete_ids })
        loading.close()
        if (code === 200) {
          this.$message({
            type: 'success',
    ...
    handleExport() {
      ...
      requestUrl += 'admin/car?target=list&export=1'
