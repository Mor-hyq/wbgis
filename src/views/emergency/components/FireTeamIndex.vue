    ...
    >
      <template slot="search">
        <el-form ref="searchForm" :model="searchForm" :size="$btnSize" inline>
          ...
          <el-form-item prop="name" :label="mylang.label">
            <el-input v-model="searchForm.name" :placeholder="`请输入${mylang.label}`" clearable />
          </el-form-item>
...
<script>
import { getFireTeamList, deleteFireTeam } from '@/api/emergency'
...
export default {
  ...
  methods: {
    initTableData({
      ...
      this.getList({ page, paginate, name, order, unit, leader })
    },
    async getList({
      order = '',
      name = '',
      ...
      this.tableData = []
      try {
        const { code, data } = await getFireTeamList({
          page, paginate, name, order, unit, leader
        })
    ...
    handleDeleteItem(row) {
      this.$confirm('即将删除您选中的' + this.mylang.fireTeam + '，确认吗？', '删除' + this.mylang.fireTeam, {
    ...
    async deleteData({ delete_ids } = {}) {
      ...
      try {
        const { code } = await deleteFireTeam({ delete_ids })
        loading.close()
        if (code === 200) {
          this.$message({
            type: 'success',
    ...
    handleExport() {
      ...
      requestUrl += 'admin/fireTeam?target=list&export=1'
