    ...
    >
      <template slot="search">
        <el-form ref="searchForm" :model="searchForm" :size="$btnSize" inline>
          <el-form-item prop="field_value_id_2" :label="mylang.typeName">
            <el-input v-model="searchForm.field_value_id_2" :placeholder="`请输入${mylang.typeName}`" clearable />
          </el-form-item>
...
<script>
import { getPipeLinkList } from '@/api/equipmentInfo'

export default {
  name: 'EquipmentListComponent',
  props: {
    selectedInfo: { // 已选中数据
      type: Array,
      default: () => []
