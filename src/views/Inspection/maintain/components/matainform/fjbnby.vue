      ...
      <div class="outer">
        <div id="print-table">
          <div class="number-input">
            <h3 class="thead">航空加油站阀井半年保养记录</h3>
            <div class="ipt">
          ...
          <div class="table-container">
            <div class="left-container">
              <div class="asset-table">
                ...
                <div class="width1">阀井水位</div>
                <div class="width1">腐蚀情况</div>
                <div class="width1">密封情况</div>
                <div class="width1">润滑情况</div>
                <div class="width1">螺栓坚固情况</div>
                ...
                >
                  ...
                  <div :key="idx+'valve_level'" class="width1 ipt-flex">
                    <span v-if="isRead" class="text">{{ list.valve_level }}</span>
                    <el-input v-else v-model="list.valve_level" :size="size" />
                  </div>
                  <div :key="idx+'corrosion'" class="width1 ipt-flex">
                    <span v-if="isRead" class="text">{{ list.corrosion }}</span>
                    <el-input v-else v-model="list.corrosion" :size="size" />
                  </div>
                  <div :key="idx+'seal'" class="width1 ipt-flex">
                    <span v-if="isRead" class="text">{{ list.seal }}</span>
                    <el-input v-else v-model="list.seal" :size="size" />
                  </div>
                  <div :key="idx+'smooth'" class="width1 ipt-flex">
                    <span v-if="isRead" class="text">{{ list.smooth }}</span>
                    <el-input v-else v-model="list.smooth" :size="size" />
                  </div>
                  <div :key="idx+'bolt'" class="width1 ipt-flex">
                    <span v-if="isRead" class="text">{{ list.bolt }}</span>
                    <el-input v-else v-model="list.bolt" :size="size" />
                  </div>
              ...
              <div class="tfooter">
                <span>备注：</span>
                <div>
                  <p>腐蚀情况主要检查阀井及管线锈蚀情况；</p>
                  <p>密封情况主要检查阀井内管线及连接设备是否存在渗油；</p>
                  <p>润滑情况主要检查阀门开关是否可以灵活启闭；</p>
                  <p>螺栓紧固情况主要检查法兰各螺栓紧固是否松动；</p>
                  <p>检查正常打“0”，有问题文字说明。</p>
                </div>
              </div>
            </div>
