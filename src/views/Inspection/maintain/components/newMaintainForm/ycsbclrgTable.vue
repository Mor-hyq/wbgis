<template>
  <div style="padding-top: 20px;">
    ...
    >
      <el-button type="primary" :size="$btnSize" @click="handlePrint(true, idx)">{{ mylang.print }}</el-button>
      <table :id="`print-table-${idx}`" class="print-table" align="center" cellspacing="0" cellpadding="0">
        <!-- <caption>{{ tableName }}</caption> -->
        <thead><th :colspan="25">{{ tableName }}</th></thead>
        <tbody>
          <tr>
          ...
          <tr>
            ...
            <td rowspan="3" colspan="2">{{ getName(table.low_check, table.low_check_text) }}</td>
            <td rowspan="3" colspan="5" style="text-align:left;vertical-align:top;">
        ...
        if (code === 200) {
          ...
          this.tableList = data.list.map(v => {
            const form = {
              ...
              remark: v.remark,
              shift: v.shift,
              low_check: +v.low_check === 1 ? 1 : 2,
              low_check_text: +v.low_check !== 1 ? v.low_check : ''
            }
